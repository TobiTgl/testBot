const express = require('express')
const app = express()
const port = 3000
const axios = require('axios');
var base64 = require('base-64');
var CronJob = require('cron').CronJob;

/*
var dateNow = Date.now()
    
var von = (dateNow/1000) + 172800

var bis = von + 300

var termin = {"termin": {"bis": von,"von": bis,"maximal": 4,"bereitsgebucht": 0,"buchbareZeitID": 5557,"gruppenID": 137,"guid": "","nochfrei": 0,"resourceID": 235,"terminArtID": 155},"terminPatient": {"ausweisnummer": null,"befundTransmitMethods": ["email","sms"],"eMail": "tobias_813@t-online.de","geburtstag": 859676400,"geschlecht": "M","handyNr": "015172662243","hausnr": "10","id": 0,"name": "Tgel","ort": "Illerrieden","plz": "89186","strasse": "Bischof-Ketteler Str","vorname": "Tobias"}}

var myString = JSON.stringify(termin)

var encodedData = base64.encode(myString);

console.log(encodedData)
*/
app.get('/', (req, res) => {
  res.send("HelloWorld!")
})

var job = new CronJob('40 12 * * 1,3,6', function(){

  var data = '';

  var dateNow = Math.round(Date.now()/1000)

  console.log(dateNow)
    
  var von = dateNow + 172800

  var bis = von + 300

  var termin = {"termin": {"bis": bis,"von": von,"maximal": 4,"bereitsgebucht": 0,"buchbareZeitID": 5557,"gruppenID": 137,"guid": "","nochfrei": 0,"resourceID": 235,"terminArtID": 155},"terminPatient": {"ausweisnummer": null,"befundTransmitMethods": ["email","sms"],"eMail": "tobias_813@t-online.de","geburtstag": 859676400,"geschlecht": "M","handyNr": "015172662243","hausnr": "10","id": 0,"name": "Tgel","ort": "Illerrieden","plz": "89186","strasse": "Bischof-Ketteler Str","vorname": "Tobias"}}

  var myString = JSON.stringify(termin)

  var encodedData = base64.encode(myString);

    var config = {
        method: 'post',
        url: 'https://termin.deineanmeldung.de/api/v2/insertappointment',
        headers: { 
        'Content-Type': 'text/plain'
        },
        data : encodedData
    };

    axios(config)
    .then(function (response) {
        console.log(JSON.stringify(response.data));
    })
    .catch(function (error) {
        console.log(error);
    });

    app.get('/letzterTermin', (req, res) => {
      res.send(encodedData)
    })

} , null, true, 'Europe/Berlin');
job.start();

app.listen(process.env.PORT || 3000)